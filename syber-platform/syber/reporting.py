...
_SEV_ORDER = {"CRITICAL": 0, "HIGH": 1, "MEDIUM": 2, "LOW": 3, "INFO": 4, "UNVERIFIED": 5}
...
<p><small><b>Severity "UNVERIFIED"</b> means the finding has NO reproducible proof (no confirmed
2xx-with-real-data capture backing it) — it is a lead for manual triage, NOT a proven vulnerability;
only findings with a curl in the reproduction section / <code>verify.sh</code> are confirmed. A
screenshot of an inaccessible (401/403/blocked) page is not a finding. Verify the confirmed ones, then
forward to the target organisation. Generated by Syber.</small></p></div>"""
...
def _reconcile_findings(findings: list[dict[str, Any]],
                        confirmed: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Tag each finding `_verified` = True iff a CONFIRMED reproduction (a curl that
    returned real data under 2xx) backs it — matched by the repro URL appearing in the
    finding's text/evidence. A HIGH/CRITICAL finding with no backing is demoted so the
    report never claims 'confirmed CRITICAL' while verify.sh is empty."""
    confirmed_urls = [str(r.get("url", "")) for r in confirmed if r.get("url")]
    out = []
    for f in findings:
        text = _finding_blob(f)
        verified = any(u and u in text for u in confirmed_urls)
        g = dict(f)
        g["_verified"] = verified
        if not verified and str(f.get("severity", "")).upper() in ("HIGH", "CRITICAL"):
            g["_reported_severity"] = f.get("severity")
            g["severity"] = "UNVERIFIED"      # rendered in the needs-triage section, not as proven
        out.append(g)
    # verified findings first
    out.sort(key=lambda x: (not x.get("_verified", False),
                            _SEV_ORDER.get(str(x.get("severity", "INFO")).upper(), 5)))
    return out


def _finding_blob(f: dict[str, Any]) -> str:
    parts = [str(f.get("summary", ""))]
    parts += [str(e) for e in (f.get("evidence_refs") or [])]
    for s in (f.get("attack_chain") or []):
        parts.append(str(s.get("description", "")))
        parts += [str(e) for e in (s.get("evidence_refs") or [])]
    return " \n ".join(parts)
...
def build_and_send(to: str | None = None, target: str = "",
    ...
    # RECONCILE findings against confirmed reproductions: a HIGH/CRITICAL finding that has
    # NO confirmed reproduction backing it is UNVERIFIED — we must NOT present it as proven
    # (that is the "CRITICAL in the email but empty verify.sh" contradiction). Demote it.
    findings = _reconcile_findings(findings, confirmed)
    ...
    verified_n = sum(1 for f in findings if f.get("_verified"))
    unverified_n = len(findings) - verified_n
    names = [a["filename"] for a in attachments]
    subject = subject or (f"[Syber] Engagement report — {target or 'target'} "
                          f"({verified_n} confirmed, {unverified_n} unverified)")
    ...
    return {"sent": True, "to": recipient, "finding_count": len(findings),
            "confirmed_findings": verified_n, "unverified_findings": unverified_n,
